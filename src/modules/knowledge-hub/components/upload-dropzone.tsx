...
export const UploadDropzone: React.FC<UploadDropzoneProps> = ({ onUploadSuccess }) => {
  ...
  const [category, setCategory] = useState('Laporan Investigasi');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const uploadedDoc = await DocumentService.uploadDocument(file, title, category);
  ...
  return (
    <div className="bg-white border border-slate-200 p-6 rounded-none shadow-sm">
      <h2 className="text-h2 mb-4 text-slate-800">Unggah Dokumen Laporan Baru</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input 
              ...
              placeholder="Contoh: Laporan Investigasi Pengadaan 2026"
              required
          ...
          <div>
            ...
            <select 
              ...
              className="w-full bg-white border border-slate-300 px-3 py-2 text-sm text-slate-900 focus:outline-none focus:border-teal-600 rounded-none shadow-xs"
            >
              <option value="Laporan Investigasi">Laporan Investigasi</option>
              <option value="Analisis Kebijakan">Analisis Kebijakan</option>
              <option value="Dokumen Hukum">Dokumen Hukum</option>
        ...
        <div className="flex justify-end pt-2">
          <button
            ...
            className={`
              px-6 py-2.5 font-roboto font-semibold text-sm text-white rounded-none flex items-center gap-2 transition-colors shadow-sm
          ...
          >
            ...
            <span>{isUploading ? 'Memproses Vektorisasi...' : 'Unggah & Proses Dokumen'}</span>
          </button>
        </div>
      </form>
    </div>
